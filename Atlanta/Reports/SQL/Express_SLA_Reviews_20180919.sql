SELECT DISTINCT --*
    ...
    ,   ((FIRST_VALUE(G.SD_IN_POSSESSION_TIME) OVER (PARTITION BY A.B1_ALT_ID,G.SD_PRO_DES ORDER BY G.SD_IN_POSSESSION_TIME DESC))
        )                                       AS "MaxInPossessionTime"
    ...
    ,   CASE
            WHEN G.G6_STAT_DD IS NOT NULL AND
                 FIRST_VALUE(G.SD_IN_POSSESSION_TIME) OVER (PARTITION BY A.B1_ALT_ID,G.SD_PRO_DES ORDER BY G.SD_IN_POSSESSION_TIME DESC) <= 0.5
...
FROM
    ...
    AND A.REC_STATUS = C.REC_STATUS
    AND C.B1_CHECKBOX_DESC = 'Workstream'
    AND C.B1_CHECKLIST_COMMENT = 'Express'
