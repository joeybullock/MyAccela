...
| Action# : N/A
|
| Notes   : On 10/12, Corey Cooper added the updated CreatedChild function he received from Accela's Jeremy Acquino.
|
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
function createChild(grp,typ,stype,cat,desc) // optional parent capId
{
	...
	if (appCreateResult.getSuccess())
			...
			if(newAddressResult.getSuccess())
				{
					...
					var districtColResult = aa.address.getAssignedAddressDistrictForDaily(itemCap.getID1(),itemCap.getID2(),itemCap.getID3(),addressId);
        ...
        //Copy GIS 
		var gisObjResult = aa.gis.getCapGISObjects(itemCap);
		if (gisObjResult.getSuccess()) 
			{
			var fGisObj = gisObjResult.getOutput();
			for (a1 in fGisObj) // for each GIS object on the Cap
