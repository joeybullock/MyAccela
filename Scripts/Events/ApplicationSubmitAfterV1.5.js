...
| Action# : N/A
|
| Notes   :  11/17/09 (Accela) Lu Hughes: Added custom function sortASITableArr
|            12/03/09 (Accela) Rocky Copeland: Modified getCapListByDetailAddress with Vernon Crandall
|            6/04/15  (Accela) Jason Plaisted: Modified sysDateMMDDYYYY with null value
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
function capIdsGetByAddr ()
	...
	logDebug("capIdsGetByAddr");
	if (streetName != null && hseNum > 0 && streetSuffix != null)
	{
 	...
 	{ 
		logDebug("**ERROR: getting similar addresses: streetName is null, streetSuffix is null, and house number is <= 0"); 
		return false; 
	}
...
//
	...
	// get caps with same address
	logDebug("getCapByAddress(ats)");
	if (ao.getStreetName() != null && ao.getHouseNumberStart() > 0 && ao.getStreetSuffix() != null)
	{
		var capAddResult = aa.cap.getCapListByDetailAddress(
			ao.getStreetName(),
			ao.getHouseNumberStart(),
			ao.getStreetSuffix(),
			ao.getZip(),
			ao.getStreetDirection(),null);
		if (capAddResult.getSuccess())
		{ 
 	...
 	{ 
		logDebug("**ERROR: getting similar addresses: streetname is null, street suffix is null, and house number start is <= 0"); 
		return false; 
	}
...
function getGISBufferInfo(svc,layer,numDistance)
 ...
 /*
 * Gets attribute value of GIS feature from given service, layer that is within
 * the given numDistance buffer radius.
 */
function getGISInfo(svc, layer, attributename, numDistance) {
   // usage:
   // x = getGISInfo("flagstaff","Parcels", "LOT_AREA", 1000);
   //

   // if numDistance is not passed, default to zero.
   if (numDistance == undefined) numDistance = 0;

   var distanceType = "feet";
   var retString;

   var bufferTargetResult = aa.gis.getGISType(svc, layer); // get the buffer
															// target
   if (bufferTargetResult.getSuccess()) {
      var buf = bufferTargetResult.getOutput();
   ...
   var gisObjResult = aa.gis.getCapGISObjects(capId); // get gis objects on
														// the cap
   if (gisObjResult.getSuccess())
      var fGisObj = gisObjResult.getOutput();
   else
   ...
   for (a1 in fGisObj) // for each GIS object on the Cap. We'll only send the
						// last value
   {
      var bufchk = aa.gis.getBufferByRadius(fGisObj[a1], numDistance, distanceType, buf);
      ...
      for (a2 in proxArr) {
         var proxObj = proxArr[a2].getGISObjects();  // if there are GIS
														// Objects here, we're
														// done
         for (z1 in proxObj) {
            var v = proxObj[z1].getAttributeValues()
...
//
	...
	for (zzz in aoArray)
		...
		var ao = aoArray[zzz];
		// get caps with same address
		logDebug("getRelatedCapsByAddress(ats)");
		if (ao.getStreetName() != null && ao.getHouseNumberStart() > 0 && ao.getStreetSuffix() != null)
		{
			var capAddResult = aa.cap.getCapListByDetailAddress(
				ao.getStreetName(),
...
function updateFee(fcode,fsched,fperiod,fqty,finvoice,pDuplicate,pFeeSeq) 
	...
	if ( !feeUpdated && adjustedQty != 0 && isNaN(adjustedQty) == false && (!invFeeFound || invFeeFound && pDuplicate=="Y") ) 
